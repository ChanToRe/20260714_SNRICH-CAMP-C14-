...
### 필수 항목
    ...
    1. **유적명**: 데이터의 출처가 되는 전체 유적의 명칭(예: 서울 풍납토성)

    2. **유구명**: 유적 내에서 시료가 출토된 개별 유구(예: 서성벽 23번 피트)

    3. **출토 맥락**: 시료가 수습된 정확한 층서적 위치나 유구 내의 구체적인 퇴적 상태(예: 3토루 내부 점토층)

    4. **BP**: 실험실에서 기계적으로 측정된 보정 전 연대 값

    5. **Error**: BP 값에 동반되는 통계적 오차 범위(±)

    6. **재질**: 측정에 사용된 유기물의 종류(목탄, 골각, 단일 탄화종자 등)

    7. **LabID**: 분석을 수행한 기관에서 부여한 고유 번호

    8. **$\delta^{13}\text{C}$**: 안정동위원소 비율, 시료의 기원(C3/C4 식물 구분 등)이나 식생 환경, 해양 리저버 효과(Marine Reservoir Effect) 등을 파악하고 교정하기 위한 목적

### 선택 항목
    1. **공간 좌표**: GIS를 활용한 공간 분포 분석이나 밀도 분석을 수행할 경우 필요한 X, Y 좌표 및 표고 데이터

    2. **유구 및 유물의 유형 및 형식 구분**: 출토된 토기의 기종과 형식, 가마의 구조적 특징과 크기, 주거지의 평면 형태 등 분석의 세부 목적에 맞게 자유롭게 항목 구성
...
#### 분석
   - `Insert` ➔ `D_Sequence` 생성

   - `Tools` ➔ `Import`를 통해 측정치 입력

   - 각 측정치들 사이에 나이테에서 확인한 간격마다 `Gap` 배치(`Others` → `Gap`)
...
#### 분석
    ...
    - `Insert` ➔ `Others` ➔ `Sequence` 클릭
 
    - 구상한 모델에 따라 `Sequence 내부에 `Phase`, `Boundary` 등의 요소 배치

    - `Tools` ➔ `Import`로 데이터 입력 후 알맞는 `Phase`에 배치

    - 단계별 지속 기간이 궁금하다면 Phase 박스 내에 `Insert` ➔ `Others` ➔ `Span` 추가
...
![R_Combine](Results/Sequence.svg)